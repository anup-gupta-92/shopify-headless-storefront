...
export default function ReviewStars({ average, count, compact = false, showCount = true, showValue = true }: ReviewRating & { compact?: boolean; showCount?: boolean; showValue?: boolean }) {
  return <span role="img" aria-label={showCount ? `Rated ${average.toFixed(1)} out of 5 from ${count} ${count === 1 ? "review" : "reviews"}` : `Rated ${average.toFixed(1)} out of 5`} className={`inline-flex items-center gap-2 ${compact ? "text-xs" : "text-sm"}`}>
    ...
    {showValue && <span className="font-semibold tabular-nums text-foreground">{average.toFixed(1)}</span>}
