...
export default async function OrderDetailPage({ params }: { params: Promise<{ order: string }> }) {
  ...
  return <main className="site-container min-h-[60vh] py-16">
    ...
    <section className="mt-8 overflow-hidden rounded-2xl border border-border bg-surface" aria-labelledby="order-items">
      ...
      <div className="divide-y divide-border">{order.lines.map((line) => <article key={line.id} className="grid gap-4 p-5 sm:grid-cols-[5rem_minmax(0,1fr)_auto] sm:items-center">
        <div className="relative aspect-square overflow-hidden rounded-lg border border-border bg-white">{line.image ? <Image src={line.image.url} alt={line.image.altText || ""} fill sizes="80px" className="object-contain p-1" /> : <span className="flex h-full items-center justify-center text-xs text-muted">No image</span>}</div>
        <div className="min-w-0"><h3 className="font-semibold">{line.name}</h3>{line.variantOptions.length > 0 && <p className="mt-1 text-sm text-muted">{line.variantOptions.map((option) => `${option.name}: ${option.value}`).join(" · ")}</p>}{line.sku && <p className="mt-1 text-xs text-muted">Product Code: {line.sku}</p>}<p className="mt-2 text-sm">Quantity: {line.quantity}{line.price && <> · {formatMoney(line.price)} each</>}</p>{Number(line.totalDiscount.amount) > 0 && <p className="mt-1 text-sm text-primary">Line discount: {formatMoney(line.totalDiscount)}</p>}</div>
